import pytest
from pydantic import BaseModel

from clients.files.files_client import FilesClient, get_files_client
from clients.files.files_schema import CreateFileRequestSchema, CreateFileResponseSchema
from fixtures.users import UserFixture


class FileFixture(BaseModel):
    """
    Данные запроса и ответа сценария 'создать файл'.
    """

    request: CreateFileRequestSchema
    response: CreateFileResponseSchema


@pytest.fixture
def files_client(function_user: UserFixture) -> FilesClient:
    """
    Фикстура создаёт клиента доступа к закрытой части API /api/v1/files
    от имени пользователя созданного фикстурой function_user

    Args:
        function_user (UserFixture): фикстура создаёт нового пользователя

    Returns:
        FilesClient: клиент доступа к закрытой части API /api/v1/files
    """
    return get_files_client(function_user.authentication_user)


@pytest.fixture
def function_file(files_client: FilesClient) -> FileFixture:
    """
    Фикстура создаёт новый файл.

    Args:
        files_client (FilesClient): фикстура создаёт клиента доступа к /api/v1/files

    Returns:
        FileFixture: данные запроса и ответа сценария 'создать файл'.
    """
    request = CreateFileRequestSchema(upload_file="./testdata/files/pytest-logo.png")
    response = files_client.create_file(request)
    return FileFixture(request=request, response=response)
