...
# Get user
get_user_headers = {"Authorization": f"Bearer {login_user_data["token"]["accessToken"]}"}
get_user_resp = httpx.get(
    f"http://localhost:8000/api/v1/users/{create_user_data["user"]["id"]}", headers=get_user_headers
)
get_user_data = get_user_resp.json()
print(get_user_resp.url, "->", get_user_resp.status_code)
print(get_user_data)
